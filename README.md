...
| `serve`                   | Runs node on `dist/server.js` which is the apps entry point 应用入口，实际运行的项目                                      |
| `watch-node`              | Runs node with nodemon so the process restarts if it crashes. Used in the main watch task         |
| `watch`                   | Runs all watch tasks (TypeScript, Sass, Node). Use this if you're not touching static assets.     |
| `test`                    | Runs tests using Jest test runner                                                                 |
| `watch-test`              | Runs tests in watch mode                                                                          |
| `build-ts`                | Compiles all source `.ts` files to `.js` files in the `dist` folder    build到dist目录                             |
| `watch-ts`                | Same as `build-ts` but continuously watches `.ts` files and re-compiles when needed 持续build               |
...
| `watch-debug`             | The same as `watch` but includes the --inspect flag so you can attach a debugger   debugger模式                |
